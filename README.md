...
Rong VideoPlayer 是一个面向 macOS 的本地学习辅助工作台，基于 **Electron + HTML5 + FFmpeg** 构建。它最初是一个高兼容本地视频播放器，现在已经扩展为集 **视频学习、Bilibili 下载、视频截图、截图库管理、Markdown 学习笔记、PDF 原生阅读、资料管理、社区分享原型** 于一体的桌面工具。
...
- 支持编辑时自动保存、本地草稿保留和重新打开时的草稿恢复。

### 学习资料上传与管理

- 支持上传 Markdown、TXT、PDF、Word、PPT、Excel、PNG、JPG、JPEG、WebP、GIF、BMP、SVG、AVIF、TIFF 等资料。
...
- 笔记列表提供“全部 / 笔记 / 资料”视图切换，资料视图支持按 PDF、图片、Office、Markdown、文本和其他类型筛选。
- 资料卡片支持打开原文件、重命名、删除和创建关联笔记。
- 新上传资料会记录文件路径、文件名、扩展名和大小等元数据；历史资料会尽量从 PDF 阅读器、图片链接和附件卡片中反推原文件路径。
- 资料重命名和删除由主进程校验，只允许操作当前笔记库 `UploadedMaterials/` 目录内的文件副本。

### 笔记库同步

- macOS 下可在设置中启用“笔记保存在 iCloud”。
- 开启后会把 `notes-db.json` 和 `UploadedMaterials/` 迁移到 iCloud Drive 下的 `Rong VideoPlayer/NotesLibrary/`。
- 如果 iCloud 中已经存在旧笔记或资料，会在切换时加载并合并旧库，避免覆盖已有学习资料。
- 旧 iCloud 资料路径会尽量归一化到当前 Mac 的 iCloud Drive 路径，减少跨设备用户名不同导致的路径失效。
...
- **笔记体验优化**：继续完善笔记模板、标签、多条件筛选、版本历史，以及更清晰的编辑/预览模式切换。
- **PDF 阅读增强**：在原生阅读器基础上继续完善阅读进度、搜索、收藏页、划线摘录和笔记引用能力。
- **资料管理继续增强**：增加资料批量操作、缺失文件检测与修复、重复资料识别、资料备注和更完整的关联笔记视图。

### 中期计划

- **电子书阅读器**：支持 EPUB、纯文本电子书等格式，提供目录、书签、高亮、摘录和阅读进度记录。
- **学习资料库检索**：建立本地全文索引，统一搜索视频文件名、笔记正文、截图 OCR 文本、PDF 文本、Office 文档文本和上传资料元数据。
...
- **Windows OneDrive 同步**：在 Windows 版本中支持 Microsoft OneDrive，检测用户 OneDrive 目录，并提供与 macOS iCloud 类似的笔记库、上传资料迁移和跨设备同步体验。
...
- 扫描媒体目录并构建目录树。
- 管理播放历史、截图数据库、笔记数据库、上传资料和资料文件安全重命名/删除。
...
- 学习笔记、Markdown 编辑器、实时预览、截图插入、资料列表和资料管理操作。
- 设置页和主题切换。
...
| `notes-db.json` | 学习笔记、资料记录、资料元数据和关联笔记数据 |
