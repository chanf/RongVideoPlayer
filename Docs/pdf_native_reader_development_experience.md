...
### 5.6 笔记列表中的 PDF 首页缩略图

PDF 进入笔记列表后，不能直接把 `.pdf-native-reader` 这段 HTML 当成摘要显示。Markdown 笔记可以天然用正文预览，图片笔记可以提取 `![alt](url)` 或 `<img src="...">`，但 PDF 笔记更适合显示首页缩略图。

实现上没有单独引入新的缩略图生成器，而是复用已有的 PDF 渲染链路：

1. 在列表渲染时解析笔记内容，识别 `.pdf-native-reader[data-pdf-path]`。
2. 兼容旧数据里的 `.pdf-container iframe[src*="/screenshot?path="]`，从 URL 参数中取出 PDF 路径。
3. 列表卡片先渲染一个轻量的 PDF 占位缩略图。
4. 后台调用 `pdf-render-page`，固定渲染 `pageIndex: 0`。
5. 渲染完成后通过 `/screenshot?path=...` 读取缓存 PNG。
6. 使用 `Image()` 预加载首页图片，加载成功后再替换占位内容。
7. 如果渲染失败，保留 PDF fallback，而不是显示源码。

这个流程和阅读器翻页的思路一致：**列表先稳定显示，再异步补齐图片内容**。缩略图生成不应该阻塞整个笔记列表，也不应该因为某个 PDF 渲染失败破坏卡片布局。

展示上也踩了一个小坑：最初把 PDF 首页做成“整页 contain 预览”，为了完整显示页面，外面加了深色背景。结果在列表卡片里会显得很像一个黑色相框，页面内容又被缩得太小，看起来既不美观，也不利于快速识别资料。

最后采用的展示原则是：

* **宽度铺满预览区域**：PDF 首页图片 `width: 100%`，与卡片缩略图区域同宽。
* **高度自动按比例展开**：不强行把整页塞进固定高度。
* **容器负责裁切**：缩略图区域固定高度，`overflow: hidden`，上下超出的内容直接裁掉。
* **优先显示页面顶部**：`object-position: top center`，因为 PDF 首页顶部通常包含标题、封面主视觉或文档开头。
* **不要显示黑色背景**：PDF 页面本身通常是白底，列表缩略图也应保持轻量干净，避免出现“黑底包白纸”的割裂感。
* **摘要改成语义提示**：PDF 卡片摘要显示“PDF 资料 · 首页缩略图预览 · 点击打开阅读器”，而不是 HTML 源码。

缩略图的目标不是完整阅读，而是帮助用户在列表中快速识别资料。因此它更像“封面裁切图”，不是“缩小版阅读器”。
...
PDF 阅读器追求“完整展示”和“长期阅读”，但列表缩略图追求“快速识别”和“信息密度”。这两个场景的设计目标不同，不能简单复用同一种展示方式。

阅读器里，页面需要完整展示，所以要根据窗口宽高自动适配，尽量让整页可见。

列表里，卡片空间有限，如果强行完整显示整页，文字会缩得很小，还可能为了补齐留白而出现难看的背景。更合理的方式是让 PDF 首页宽度铺满卡片，按固定高度裁切顶部区域，把它当作资料封面使用。
